...
def summarize_and_tag(text: str, tgi_url: str):
    """
    Appelle TGI pour résumer, taguer le sentiment et extraire des thèmes.
    Retourne un dict JSON {resume, sentiment, themes}.
    """
    prompt = (
...
def embed_texts(texts: list[str], tei_url: str) -> list[list[float]]:
    ...
    """
    r = requests.post(f"{tei_url}/embed", json={"inputs": texts, "truncate": True}, timeout=60)
    r.raise_for_status()
    data = r.json()
...
def enrich_with_llm_and_embedding(texte: str, tgi_url: str | None = None, tei_url: str | None = None):
    ...
    """
    tgi = tgi_url or os.getenv("TGI_URL", "http://tgi:80")
    tei = tei_url or os.getenv("TEI_URL", "http://tei:80")
