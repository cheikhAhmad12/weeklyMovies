...
def insert_review(conn, film_id, source_id, row, enriched, emb) -> bool:
    """
    Retourne True si une ligne a été insérée,
    False si le ON CONFLICT a fait DO NOTHING.
    """
    with conn.cursor() as cur:
        cur.execute("""
        INSERT INTO fact_critique (film_id, source_id, auteur, date_publication, note_numeric,
        ...
        """, (
            film_id, source_id, row.get("auteur"), row.get("note"), row["texte"],
            enriched.get("resume"), enriched.get("sentiment"), enriched.get("themes"),
            emb, row["url"], row["hash_critique"]
        ))
        # rowcount = 1 si insert, 0 si conflit
...
def fake_scrape_reviews():
    ...
    critiques.append({
        ...
        "texte": texte3,
        "url": url3,
        "hash_critique": make_hash(texte3, url3),
    })
...
def run_weekly(limit: int = 10):
    ...
    for row in reviews:
        try:
            ...
            print(f"   DEBUG hash={row['hash_critique']}, url={row['url']}")
            # 4.3 Insertion fact_critique
            ok = insert_review(conn, film_id, source_id, row, enriched, emb)
            if ok:
